        ...
        <table className="min-w-full">
          ...
          <tbody>
            {data.map((row, index) => {
              ...
              const percentChange = isNaN(parseFloat(row.percent_change)) ? 0 : parseFloat(row.percent_change);
              return (
              <tr 
                key={index} 
              ...
              >
                ...
                <td className="py-3 px-6 text-gray-800">{isNaN(price) ? 'N/A' : price.toFixed(2)}</td> {/* Ensure fallback for invalid price */}
                <td className="py-3 px-6 text-gray-800">{row.daily_change}</td>
                <td className="py-3 px-6 text-gray-800">{isNaN(percentChange) ? 'N/A' : percentChange.toFixed(2)}%</td> {/* Ensure fallback for invalid percent change */}
